...
const latencyPoints = [
  42, 38, 55, 34, 61, 48, 72, 56, 68, 82, 61, 88, 74, 80, 92, 77, 84, 91, 78,
  96, 85, 90, 76, 88,
];
...
const LineChart = ({ points, color = "#00D4FF", gradId }) => {
  ...
  return (
    ...
    >
      ...
      <polygon
        points={`0,${svgH} ${coords.join(" ")} ${svgW},${svgH}`}
        fill={`url(#${gradId})`}
      />
      <polyline
        points={coords.join(" ")}
        ...
        return (
          <g key={i}>
            <rect
              ...
              height={barH}
              rx="3"
              fill={`rgba(0,212,255,${0.3 + (h / max) * 0.5})`}
            />
            <text
              x={x + 11}
              y="108"
...
const AnalyticsPage = () => {
  ...
  const handlePDFExport = (range, sections) => {
    ...
    const drawFooter = (pageNum, totalPages) => {
      ...
      doc.text(
        "Generated by NodeSlix Analytics Platform",
        MARGIN,
        PAGE_HEIGHT - 12,
      );
      doc.text(
    ...
    doc.text(
      sections.includes("Infrastructure Health") ? "98.7%" : "Excluded",
      MARGIN + 6,
      currentY + 15,
    );
    ...
    doc.text(
      sections.includes("Latency Statistics") ? "12 ms" : "Excluded",
      MARGIN + cardW + 14,
      currentY + 15,
    );
    ...
    doc.setTextColor(0, 130, 255);
    doc.text(
      sections.includes("Bandwidth Utilization") ? "72%" : "Excluded",
      MARGIN + 6,
      currentY + 15,
    );
    ...
    doc.text(
      sections.includes("AI Optimization Metrics") ? "96%" : "Excluded",
      MARGIN + cardW + 14,
      currentY + 15,
    );
    ...
    if (sections.includes("Latency Statistics")) {
      ...
      latencyPoints.forEach((val, i) => {
        ...
        const py =
          chartY + chartH - 4 - ((val - minVal) / valRange) * (chartH - 8);
    ...
    doc.save(
      `nodeslix_analytics_report_${range.toLowerCase().replace(/ /g, "_")}.pdf`,
    );
  };
  ...
  const handleCSVExport = (range, sections) => {
    ...
    if (
      sections.includes("Infrastructure Health") ||
      sections.includes("Network Performance") ||
      sections.includes("Bandwidth Utilization")
    ) {
  ...
  const handleJSONExport = (range, sections) => {
    ...
    if (sections.includes("Bandwidth Utilization")) {
      ...
      reportData.dataSets.monthlyBandwidthUsage = bandwidthBars.map(
        (bar, i) => ({
          month: months[i],
          throughputTbps: bar,
        }),
    ...
    if (sections.includes("Topology Summary")) {
      reportData.dataSets.topologySummary = {
        description:
          "Dynamic network topologies analyzed across 5G nodes, Edge gateways and Core layers.",
        activeLayers: [
          "Core Systems",
          "5G Network",
          "Edge Gateways",
  ...
  const handleExcelExport = (range, sections) => {
    ...
    if (
      sections.includes("Traffic Analysis") ||
      sections.includes("Bandwidth Utilization") ||
      sections.includes("Latency Statistics")
    ) {
      const trafficData = [
    ...
    XLSX.writeFile(
      wb,
      `nodeslix_analytics_report_${range.toLowerCase().replace(/ /g, "_")}.xlsx`,
    );
  };
    ...
    <div className="p-5 md:p-6 space-y-7">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-nodeslix-muted">
            Deep insights into performance, capacity, and efficiency trends.
          </p>
        </div>
        ...
        {topMetrics.map((m, i) => (
          ...
          >
            <div className="flex items-center justify-between">
              <p className="text-[10px] text-nodeslix-muted/70 uppercase tracking-wider font-semibold">
                {m.label}
              </p>
              <span
                className={`flex items-center gap-0.5 text-[10px] font-bold ${m.positive ? "text-emerald-400" : "text-red-400"}`}
              >
                {m.positive ? (
                  <TrendingUp size={10} />
                ) : (
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        ...
        >
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-[10px] text-nodeslix-muted/60 mt-0.5">
            ...
            <div className="flex-1 space-y-3">
              {donutSegments.map((seg) => (
                <div key={seg.label} className="flex items-center gap-3">
                  <span
                    className="block rounded-full size-2 shrink-0"
                    style={{ backgroundColor: seg.color }}
                  />
                  <span className="flex-1 text-xs text-nodeslix-muted">
                    {seg.label}
                  </span>
        ...
        >
          <div>
            ...
            <p className="text-[10px] text-nodeslix-muted/60 mt-0.5">
              Rolling efficiency rating per pipeline stage
            </p>
          </div>
          <div className="space-y-3">
            {[
              ...
              <div key={s.label} className="flex items-center gap-3">
                <span className="text-[10px] text-nodeslix-muted w-36 shrink-0 truncate">
                  {s.label}
                </span>
                ...
                <span className="text-[10px] font-bold text-white w-8 text-right shrink-0">
                  {s.score}%
                </span>
              </div>
            ))}
        ...
        <div className="overflow-hidden surface-card">
          ...
          {tableRows.map((row, i) => (
            ...
            >
              ...
              <span className="text-xs text-right text-nodeslix-muted">
                {row.throughput}
              </span>
              <span className="text-xs text-right text-nodeslix-muted">
                {row.latency}
              ...
              <div className="flex justify-end">
                <span
                  className={`text-[9px] font-bold px-2 py-0.5 rounded-full border ${statusStyle[row.status]}`}
                >
                  {row.status}
                </span>
              </div>
              ...
              {isGenerating ? (
                ...
                <div className="flex flex-col items-center justify-center p-8 space-y-6 text-center">
                  <div className="relative flex items-center justify-center">
                    ...
                    <div className="absolute size-8 rounded-full bg-[#00D4FF]/10 border border-[#00D4FF]/30 flex items-center justify-center">
                      <Loader2
                        size={16}
                        className="text-[#00D4FF] animate-spin"
                      />
                    </div>
                  </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <p className="text-xs text-nodeslix-muted">
                      Assembling secure client-side enterprise data datasets...
                    </p>
                  </div>
                  ...
                  <div className="w-full max-w-xs space-y-2">
                    ...
                    <div className="flex justify-between text-[10px] font-bold text-nodeslix-muted px-0.5">
                      ...
                      <span className="animate-pulse text-[#00D4FF]">
                        Compiling
                      </span>
                    </div>
                  </div>
                ...
                <div className="p-6 space-y-6 md:p-8">
                  <div>
                    ...
                    <p className="text-xs text-nodeslix-muted mt-1.5">
                      Choose the format and data range for your report.
                    </p>
                  </div>
                  ...
                  <div className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div className="space-y-1.5">
                        <label className="text-[10px] font-bold text-nodeslix-muted uppercase tracking-wider">
                          Report Format
                        </label>
                        <select
                          value={exportFormat}
                        ...
                        {[
                          ...
                          return (
                            ...
                            >
                              ...
                              <span className="text-[11px] text-nodeslix-muted hover:text-white transition-colors select-none">
                                {sec}
                              </span>
                            </label>
                          );
        ...
        {toast && (
          ...
          >
            ...
            <div className="flex-1">
              ...
              <p className="text-[11px] text-nodeslix-muted mt-0.5 leading-normal">
                {toast.message}
              </p>
            </div>
            <button
